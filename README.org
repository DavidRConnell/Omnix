...
** CAPFs
Omnix provides completion at point functions (CAPFs) for some link types.
These can be turned on by adding the relevant setup functions to `org-mode-hook`.

#+begin_src elisp
  (add-hook 'org-mode-hook #'omnix-acronym-setup-capf)
  (add-hook 'org-mode-hook #'omnix-color-setup-capf)
#+end_src
